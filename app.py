...
def get_conversation_chain(vectorstore):
    ...
    return ConversationalRetrievalChain.from_llm(
        llm=llm, retriever=vectorstore.as_retriever(search_kwargs={"k": k_value}), memory=memory
    )
    ...
    with st.sidebar:
        ...
        if pdf_docs and process_button:
            with st.spinner("Processing..."):
                ...
                if 'user_question' in st.session_state:  # Check if there's a user question from session state
                    st.session_state.conversation = get_conversation_chain(vectorstore, st.session_state.user_question)
                else:
